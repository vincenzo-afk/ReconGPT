...
Read the [contribution guide](./CONTRIBUTING.md) before opening an issue or pull request. It documents the verified setup, privacy and source-policy boundaries, focused branch workflow, required checks, and review expectations.

Use short, imperative commit subjects. Conventional Commit prefixes such as `feat:`, `fix:`, `test:`, and `docs:` are preferred because they make release history easier to read. A pull request should explain analyst-facing impact, source or safety implications, tests run, and any migration or secret requirement. GitHub Actions verifies tests, TypeScript, and the Vercel build for pushes and pull requests targeting `main`.
